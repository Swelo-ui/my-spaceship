...
const getPlanet1Distance = (p_vec: number[] | Float32Array, uniforms: any, t: number) => {
    ...
    // PERF: Precompute cos/sin of base rotation angle when pulse is zero (common case)
    const hasPulse = pulse !== 0.0;
    const baseC = Math.cos(rot);
    const baseS = Math.sin(rot);

    while (i > 0.05) {
        // PERF: Skip expensive sin/cos when pulse is 0 (default)
        let c: number, s: number;
        if (hasPulse) {
            const angle = rot + Math.sin(t + temp_q[1] * 5.0) * pulse;
            c = Math.cos(angle);
            s = Math.sin(angle);
        } else {
            c = baseC;
...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
    ...
    useEffect(() => {
        const loadDefaultSession = async () => {
            try {
                const response = await fetch('/sessions/shader-session-1.json');
                if (!response.ok) throw new Error('Failed to load default session');
                const session: SessionState = await response.json();

                if (session.shaderCode) setActiveShaderCode(session.shaderCode);
                if (session.sliders) setSliders(session.sliders);
                if (session.uniforms) setUniforms(session.uniforms);
                if (session.cameraControlsEnabled !== undefined) setCameraControlsEnabled(session.cameraControlsEnabled);
                if (session.terraformConfig) setTerraformConfig(session.terraformConfig);
                if (session.controlConfig) setControlConfig(session.controlConfig);
                if (session.soundConfig) setSoundConfig(session.soundConfig);
                if (session.shipConfig) setShipConfig(session.shipConfig);
                if (session.canvasSize) setCanvasSize(session.canvasSize);
                if (session.viewMode) setViewModeState(session.viewMode);
                if (session.isHdEnabled !== undefined) setIsHdEnabled(session.isHdEnabled);
                if (session.isFpsEnabled !== undefined) setIsFpsEnabled(session.isFpsEnabled);
                if (session.isHudEnabled !== undefined) setIsHudEnabled(session.isHudEnabled);
                if (session.collisionThresholdRed !== undefined) setCollisionThresholdRed(session.collisionThresholdRed);
                if (session.collisionThresholdYellow !== undefined) setCollisionThresholdYellow(session.collisionThresholdYellow);
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const gameLoop = (timestamp: number) => {
            ...
            // PERF FIX: Use ref for comparison instead of stale closure value
            setTerraformPower(prev => {
                const next = terraformPowerRef.current;
